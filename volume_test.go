...
func TestFindHostIOLimitByName(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(fmt.Errorf("not found")).Once()
...
func TestCreateLun(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(fmt.Errorf("storage pool not found")).Once()
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(fmt.Errorf("volume already exists")).Once()
	_, err = testConf.client.CreateLun(ctx, volName, testConf.poolID, "Description", 2368709120, 0, hostIOLimitID, true, false)
	if err == nil {
...
func TestFindVolumeByName(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(fmt.Errorf("volume not found")).Once()
...
func TestListVolumes(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(nil).Run(func(args mock.Arguments) {
...
func TestExportVolume(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("FindHostByName", ctx, testConf.nodeHostName).Return(&types.Host{
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(&types.Error{
...
func TestExpandVolumeTest(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).ExpectedCalls = nil
	ctx := context.Background()
	testConf.client.getAPI().(*mocksapi.Client).On("FindVolumeByID", ctx, "volID").Return(&types.VolumeContent{SizeTotal: 5368709120}, nil).Once()
	testConf.client.getAPI().(*mocksapi.Client).On("FindVolumeByID", ctx, "dummy_vol_sv_1").Return(nil, errors.New("unable to find volume Id")).Once()
	testConf.client.getAPI().(*mocksapi.Client).On("executeWithRetryAuthenticate", ctx, "POST", "api/modify/lun/volID", mock.Anything, nil).Return(nil).Once()
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
...
func TestCreateCloneFromVolume(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil).Times(2)
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(fmt.Errorf("volume with a same exists")).Once()
...
func TestModifyVolumeExportTest(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", mock.Anything, "POST", "/api/instances/storageResource/dummy_vol_1/action/modifyLun", mock.Anything, mock.Anything).Return(fmt.Errorf("volume not found")).Once()
...
func TestDeleteVolumeTest(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(nil).Times(3)
	err = testConf.client.DeleteVolume(ctx, volID)
	assert.Nil(t, err)
	...
	// Mock the FindVolumeByID method to return voume not found error
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(errors.New(DependentClonesErrorCode)).Once()
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", anyArgs...).Return(nil).
		Run(func(args mock.Arguments) {
...
func TestGetMaxVolumeSizeTest(t *testing.T) {
	...
	testConf.client.getAPI().(*mocksapi.Client).On("DoWithHeaders", mock.Anything, "GET", "/api/types/systemLimit/instances/dummy_name", mock.Anything, mock.Anything).Return(fmt.Errorf("invalid systemLimitID")).Once()
