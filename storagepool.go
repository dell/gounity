...
func (c *UnityClientImpl) FindStoragePoolByName(ctx context.Context, poolName string) (*apitypes.StoragePool, error) {
	if len(poolName) == 0 {
		return nil, errors.New("poolName shouldn't be empty")
	}
	spResponse := &apitypes.StoragePool{}
...
func (c *UnityClientImpl) FindStoragePoolByID(ctx context.Context, poolID string) (*apitypes.StoragePool, error) {
	if len(poolID) == 0 {
